...
<xsl:template name="textstyleclass">
  <xsl:choose>
    ...
    <xsl:when test="@styleclass = 'Heading1'"><xsl:attribute name="style">font-family:Arial; font-size:14pt; font-weight:bold; font-style:normal; text-decoration:none; text-transform:none; vertical-align:baseline; color:#ffffff; background-color:transparent; letter-spacing:normal; letter-scaling:100%; <xsl:value-of select="@style"/></xsl:attribute></xsl:when>
